...
const forms = document.querySelectorAll('form');
    ...
    //     'Content-type':'application/json'
    //   }
    // }) // чтобы делать другие запросы(post,put) нужно добавить небольшие настройки для нашего fetcha
    //   .then(response => response.json()) // response.json этот метод возьмёт и превратит данные в формате json в самый обычный  js объект который можно дальше юзать 
    //   .then(json => console.log(json));
      // когда нам возвращается promise из какой-то функции или с какой-то перененной в которую обёрнута функция то мы его можем обработать с помощью цепочки .then // fetch() современная и крайне удобная технология,которая позволяет общаться с сервером и она построенна у нас на promise
});

//Tegs

// API - Application programmer interface(набор данных и возможностей которые предоставляет нам какаой-то готовое решение)
